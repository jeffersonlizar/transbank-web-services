...
def init(request):
    ...
    button = "</br></br><form action=" + client[
        "urlWebpay"] + " method='post'><input type='hidden' name='TBK_TOKEN' value=" + client[
...
def OneClickAuthorize(request):
    ...
    if request.POST.get("token_ws", "") is None:
        ...
        button = "</br></br><form action=" + next_page + " method='post'><input type='hidden' name='buyOrder' value=" + str(
            buyOrder) + "><input type='hidden' name='tbkUser' value=" + str(
            tbkUser) + "><input type='submit' value='Continuar'></form>"
        link = "<a href='../default/index'>&laquo; volver a index</a>";
