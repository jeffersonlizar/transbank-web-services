...
def init(request):
    ...
    try:
        ...
        client = webpay.getCompleteTransaction().initCompleteTransaction(amount, buyOrder, sessionId,
                                                                         cardExpirationDate, cvv, cardNumber)
        ...
        result = "<p style='font-size: 100% background-color:lightgrey'><strong>result</strong><br><br><br/>" + text + ". " + str(
            e) + "</p>"
        return HttpResponse(title + step + result + link)

    request = "<p style='font-size: 100% background-color:lightyellow'><strong>request</strong><br><br><br/>" + str(
        request_dict) + "</p>"
    result = "<p style='font-size: 100% background-color:lightgrey'><strong>result</strong><br><br><br/>" + str(
        client) + "</p>"
    button = "</br></br><form action=" + next_page + " method='post'><input type='hidden' name='buyOrder' value=" + str(
        buyOrder) + "><input type='hidden' name='token_ws' value=" + client[
                 "token"] + "><input type='submit' value='Continuar'></form>"
    link = "<a href='../default/index'>&laquo volver a index</a>"
...
def queryshare(request):
    ...
    if request.POST.get("token_ws", "") is None:
        ...
        result = "<p style='font-size: 100% background-color:lightgrey'><strong>result</strong><br><br><br/>" + str(
            client) + "</p>"
        button = "</br></br><form action=" + next_page + " method='post'><input type='hidden' name='queryId' value=" + str(
            client.queryId) + "><input type='hidden' name='buyOrder' value=" + buyOrder + "><input type='hidden' name='token_ws' value=" + token + "><input type='submit' value='Continuar'></form>"
        link = "<a href='../default/index'>&laquo volver a index</a>"
...
def authorize(request):
    ...
    if request.POST.get("token_ws", "") is None:
        ...
        try:
            ...
            client = webpay.getCompleteTransaction().authorize(token, buyOrder, gracePeriod, idQueryShare,
                                                               deferredPeriodIndex)
        ...
        result = "<p style='font-size: 100% background-color:lightgrey'><strong>result</strong><br><br><br/>" + str(
            client) + "</p>"
        button = "</br></br><form action=" + next_page + " method='post'><input type='hidden' name='authorizationCode' value=" + \
                 client.detailsOutput[0][
                     'authorizationCode'] + "><input type='hidden' name='idQueryShare' value=" + idQueryShare + "><input type='hidden' name='buyOrder' value=" + buyOrder + "><input type='hidden' name='token_ws' value=" + token + "><input type='submit' value='Continuar'></form>"
        link = "<a href='../default/index'>&laquo volver a index</a>"
...
def end(request):
    ...
    button = "</br></br><form action=" + next_page + " method='post'><input type='hidden' name='authorizationCode' value=" + authorizationCode + "><input type='hidden' name='buyOrder' value=" + buyOrder + "><input type='hidden' name='token_ws' value=" + message + "><input type='submit' value='Anular transacci&oacuten'></form>"
    link = "<a href='../default/index'>&laquo volver a index</a>"
...
def nullify(request):
    ...
    try:
        ...
        client = webpay.getNullifyTransaction().nullify(authorizationCode, authorizedAmount, buyOrder, nullifyAmount,
                                                        commercecode)

        message = "Transacci&oacuten Finalizada"
