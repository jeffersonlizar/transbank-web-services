...
def init(request):
    ...
    title = '<h1>Ejemplos Webpay - Transaccion Mall Normal</h1>'
    ...
    try:
        ...
        client = webpay.getMallNormalTransaction().initTransaction(amount, buyOrder, sessionId, urlReturn, urlFinal,
                                                                   stores)
    ...
    request = "<p style='font-size: 100% background-color:lightyellow'><strong>request</strong><br><br><br/>" + str(
        request_dict) + ", " + str(stores) + "</p>"
    result = "<p style='font-size: 100% background-color:lightgrey'><strong>result</strong><br><br><br/>" + str(
        client) + "</p>"
    button = "</br></br><form action=" + client["url"] + " method='post'><input type='hidden' name='token_ws' value=" + \
             client["token"] + "><input type='submit' value='Continuar'></form>"
    link = "<a href='../default/index'>&laquo volver a index</a>"
...
""" Llamada a pagina /result (Permite obtener el resultado de la transaccion) """
...
def result(request):
    ...
    if request.POST.get("token_ws", "") is None:
        ...
        try:
            ...
            client = webpay.getMallNormalTransaction().getTransaction(token)

            if (client.detailOutput[0]['responseCode'] == 0):

                message = "Pago ACEPTADO por webpay (se deben guardar datos para mostrar voucher)"

                script = "<script>localStorage.setItem('authorizationCode', " + str(
                    client.detailOutput[0]['authorizationCode']) + ")</script>"
                script = script + "<script>localStorage.setItem('commercecode', " + str(
                    client.detailOutput[0]['commerceCode']) + ")</script>"
                script = script + "<script>localStorage.setItem('amount', " + str(
                    client.detailOutput[0]['amount']) + ")</script>"
                script = script + "<script>localStorage.setItem('buyOrder', " + str(
                    client.detailOutput[0]['buyOrder']) + ")</script>"
        ...
        result = "<p style='font-size: 100% background-color:lightgrey'><strong>result</strong><br><br><br/>" + str(
            client) + "</p>"
        button = "</br></br><form action=" + client[
            "urlRedirection"] + " method='post'><input type='hidden' name='token_ws' value=" + token + "><input type='submit' value='Continuar'></form>"
        link = "<a href='../default/index'>&laquo volver a index</a>"
...
def end(request):
    ...
    if (request.POST.get('token_ws') != None):
        ...
        next_page = "http://127.0.0.1:8000/sample/tbk_mall_normal/" + "nullify"
        ...
        nullify = nullify + "<input type='submit' value='Anular Transacci&oacuten &raquo'></form>"
...
def nullify(request):
    ...
    try:
        ...
        authorizedAmount = request.POST.get('amount')
